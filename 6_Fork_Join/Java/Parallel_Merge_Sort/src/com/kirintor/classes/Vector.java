...
public class Vector {
	...
	public void mergeSort(int startIndex, int finishIndex){
		Vector result = new Vector(finishIndex-startIndex, 0);
		int h = (startIndex+finishIndex)/2;
		ArrayList<Integer> array1 = new ArrayList<Integer>(h-startIndex);
		ArrayList<Integer> array2 = new ArrayList<Integer>(finishIndex-h);
		ArrayList<Integer> resultList = new ArrayList<Integer>(result.getN());
		
		for(int i = startIndex; i<h; i++){
			array1.add(Integer.valueOf(data[i]));
		}
		for(int j = h; j<finishIndex; j++){
			array2.add(Integer.valueOf(data[j]));
		}
